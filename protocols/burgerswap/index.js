...
const factoryABI = JSON.parse(
  fs.readFileSync("./abi/burgerswapFactoryABI.json")
);
const poolABI = JSON.parse(fs.readFileSync("./abi/burgerswapPoolABI.json"));
...
// ROUTER
// ?
const BURGERSWAP_FACTORY = "0x8a1E9d3aEbBBd5bA2A64d3355A48dD5E9b511256"; // mainnet

async function init(web3Obj) {
  web3 = web3Obj;
  factoryContract = new web3.eth.Contract(factoryABI, BURGERSWAP_FACTORY);
  ...
  if (tokens) {
    ...
    await loadTokens();
    console.error("[BURGERSWAP]", tokens_map.size, "pre loaded tokens");
  }

  console.log("[BURGERSWAP] Loading Pools and Pairs");
  const pools = await loadListFromMemory("POOLS:BURGERSWAP");

  if (!pools) {
    console.error("[BURGERSWAP] No pools in memory, loading it...");
    await populateSwapPools(0);
    console.error(
      "[BURGERSWAP] All pools are loaded, exiting and starting again..."
    );
    process.exit();
  }
  ...
  console.log("[BURGERSWAP] Started", totalPools, "pools");
  ...
  schedule.scheduleJob("* * * * *", function () {
    console.log("[BURGERSWAP] Checking for new pools");
    factoryContract.methods
      ...
      .then((allPairsLength) => {
        if (totalPools != allPairsLength) {
          ...
          if (difference < 0) {
            console.error(
              "[BURGERSWAP] BURGERSWAP POOLS ARE INCONSISTENT:",
              difference
            );
          } else {
            console.log(
              "[BURGERSWAP] New pools found:",
              difference,
              " starting from:",
              totalPools
            );
            populateSwapPools(totalPools).then(() => {
              console.log(
                "[BURGERSWAP] New pools updated, new total is:" + totalPools
              );
            });
          }
        } else {
          console.log("[BURGERSWAP] All pools are update:", totalPools);
        }
      });
  });
}
...
const getPoolData = async (poolID) => {
  try {
    ...
    if (token0 === null) {
      ...
      console.error("[BURGERSWAP] Token not found:", token0Addr);
      return;
    }
    ...
    if (token1 === null) {
      ...
      console.error("[BURGERSWAP] Token not found:", token1Addr);
      return;
    }
    ...
    console.error("[BURGERSWAP] getPoolData", error.name + ":" + error.message);
  }
};
...
async function populateSwapPools(beginPoolIndex) {
  try {
    ...
    for (var i = beginPoolIndex; i < allPairsLength; i++) {
      ...
      if (!pair) {
        console.info("[BURGERSWAP] Problem creating a pair for pool", poolID);
        continue;
      }
...
const createNewPool = async (poolID) => {
  ...
  if (!pair) {
    ...
    console.info(
      "[BURGERSWAP] Problem creating a pair for pool",
      poolID + "\n"
    );
    return false;
  }
  ...
  try {
    ...
    if (pool_pairs_map.has(poolID)) {
      ...
      await redisClient.set(
        "ACTIVE:BURGERSWAP:" + poolID,
        JSON.stringify(pair)
      );
      await redisClient.expire("ACTIVE:BURGERSWAP:" + poolID, 86400);
...
const swapWatcher = async () => {
  ...
  web3.eth
    ...
    .on("connected", async () => {
      console.log(
        "[BURGERSWAP] Connected to Binance Smart Chain: Listening for syncs on CHAIN ID:" +
          chainId
      );
    })
    .on("data", async (sync) => {
