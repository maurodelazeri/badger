...
async function init(web3Obj) {
  ...
  const pools = await loadListFromMemory("POOLS:BALANCER");
...
const createNewPool = async (poolID) => {
  ...
  await redisClient.hset("POOLS:BALANCER", poolID, JSON.stringify(pair));
...
async function streamWorker(sync) {
  ...
  try {
    ...
    ZMQ.zmqSendMsg("TICKERS_BALANCER", poolID, JSON.stringify(ticker));

    await redisClient.hset("POOLS:BALANCER", poolID, JSON.stringify(pair));

    await redisClient.set("ACTIVE:BALANCER:" + poolID, JSON.stringify(pair));
    await redisClient.expire("ACTIVE:BALANCER:" + poolID, 86400);
